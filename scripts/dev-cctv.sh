...
PORT="${PORT:-4173}"
HOST="${HOST:-0.0.0.0}"
CCTV_SOURCES_FILE="${CCTV_SOURCES_FILE:-config/cctv_sources.windy.json}"
CCTV_WINDY_MAX_SOURCES="${CCTV_WINDY_MAX_SOURCES:-48}"
...
if [[ -f "${CCTV_SOURCES_FILE}" ]]; then
  ...
  echo "CCTV_SOURCES_FILE: ${CCTV_SOURCES_FILE} (missing, using the live Windy Webcams sweep)"
fi
